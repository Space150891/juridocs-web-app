...
class LanguageListPage extends Component {
  ...
  saveSettings(data) {
    let subjectPromise = this.props.updateSetting(data.form[settings.keys.SHARE_EMAIL_SUBJECT]);
    let messagePromise = this.props.updateSetting(data.form[settings.keys.SHARE_EMAIL_MESSAGE]);
    ...
    return (
      <div className="LanguageListPage">
        ...
        <Topbar currentLanguage={this.props.currentLanguage} handleLangChange={this.props.handleLangChange}>
          <div className="title">
            <Link to="/settings/shareEmail">{strings.get('App.settings.title')}</Link>
          </div>
        </Topbar>
        <SubTopbar>
          <SettingsTabs />
